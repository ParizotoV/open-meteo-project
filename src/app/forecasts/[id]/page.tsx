...
const Details: React.FC<DetailsProps> = ({ hiddenCharts = false }) => {
  const { push } = useRouter();
  const { weathers, initValues, updateWeather, removeWeather } =
    useGlobalState();
  ...
  useEffect(() => {
    if (weathers.length === 0) {
      ...
      const weather = weathers.find(
        (weather) => weather.uuid === location.pathname.split("/")[2]
      );
      ...
      <div className="flex p-12 box-border flex-col gap-3 max-w-[550px] w-full">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Infos
            title="UV Index"
            subtitle={`${
              weather?.daily?.uv_index_max.sort()[
                weather?.daily?.uv_index_max.length - 1
              ]
          ...
          <Infos
            title={
              <div className="flex items-center justify-center gap-2 py-3">
                <Sunset width={28} />
                <span className="text-lg">
                  {moment(weather?.daily?.sunset[0]).format("HH:mm")}
                </span>
              </div>
            }
          />
