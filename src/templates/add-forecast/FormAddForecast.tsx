...
type FormAddForecastProps = {
  getDetailsLocation: (latitude: number, longitude: number) => Promise<AxiosResponse<any, any>>
}
...
const FormAddForecast: React.FC<FormAddForecastProps> = ({ getDetailsLocation }) => {
  ...
  return (
    <form role='form' className='flex flex-col gap-5' onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className='flex flex-col gap-2'>
        ...
        />
        {errors.longitude && (
          <span className='text-xs text-red-600'>{errors.longitude.message}</span>
        )}
      </div>
      ...
      >
        {loading ? (
          ...
          <h3 className='text-white text-lg'>Add</h3>
        )}
      </button>
    </form>
  )
